                ...
                for (const action_trace of action_traces) {
                    if (action_trace[0] === 'action_trace_v0') {
                        ...
                        if (action_blacklist.has(`${queue_prefix}::${action['act']['account']}::*`)) {
                            ...
                            const status = await processAction(ts, action, trx_id, block_num, producer, _actDataArray, _processedTraces);
                            if (status) {
                                action_count++;
                            }
                        }
...
async function processAction(ts, action, trx_id, block_num, prod, _actDataArray, _processedTraces) {
    let act = action['act'];
