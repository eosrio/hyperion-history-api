...
export default class StateReader extends HyperionWorker {
    isLiveReader = process.env['worker_role'] === 'continuous_reader';
    baseRequest;
    abi;
    qStatusMap = {};
    recovery = false;
    ...
    stageOneDistQueue;
    blockReadingQueue;
    range_size = parseInt(process.env.last_block) - parseInt(process.env.first_block);
    completionMonitoring;
    types;
    local_block_num = parseInt(process.env.first_block, 10) - 1;
    queueSizeCheckInterval = 5000;
    ...
    async processIncomingBlocks(block_array) {
        if (this.abi) {
    ...
    async logForkEvent(starting_block, ending_block, new_id) {
    ...
    startWS() {
        this.ship.connect(this.blockReadingQueue.push, this.handleLostConnection.bind(this), () => {
