...
export default class DSPoolWorker extends HyperionWorker {
    ...
    async fetchAbiHexAtBlockElastic(contract_name, last_block, get_json, fetch_offset) {
        try {
    ...
    async deserializeActionAtBlock(action, block_num) {
        ...
        if (contract) {
            if (contract[0].actions.has(action.name)) {
                try {
                    const deserialized_act = Serialize.deserializeAction(contract[0], action.account, action.name, action.authorization, action.data, this.txEnc, this.txDec);
