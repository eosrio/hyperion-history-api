...
export default class HyperionParser extends BaseParser {
    flatten = false;
    async parseAction(worker, ts, action, trx_data, _actDataArray, _processedTraces, full_trace, usageIncluded) {
        // check filters
    ...
    async parseMessage(worker, messages) {
                ...
                if (worker.conf.whitelists.root_only) {
                    try {
                        ...
                        if (worker.conf.settings.ds_profiling)
                            ds_times['packed_trx'] = process.hrtime.bigint();
                        ...
                        // store time diff
                        if (worker.conf.settings.ds_profiling)
                            ds_times['packed_trx'] = Number(process.hrtime.bigint() - ds_times['packed_trx']) / 1000;
                    }
                    catch (e) {
                        console.log(e);
                        allowProcessing = true;
    ...
    async flattenInlineActions(action_traces) {
