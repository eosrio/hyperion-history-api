...
import { debugLog, getLastIndexedABI, getLastIndexedBlock, getLastIndexedBlockByDelta, getLastIndexedBlockByDeltaFromRange, getLastIndexedBlockFromRange, hLog, messageAllWorkers } from "../helpers/common_functions.js";
import pm2io from '@pm2/io';
import { createWriteStream, existsSync, mkdirSync, readFileSync, symlinkSync, unlinkSync, writeFileSync } from "fs";
...
export class HyperionMaster {
    ...
    liveBlockQueue;
    readingPaused = false;
    readingLimited = false;
    // Hyperion Hub Socket
    hub;
    // Timers
    contractMonitoringInterval;
            ...
            'completed': (msg) => {
                if (msg.id === this.doctorId.toString()) {
                    ...
                    if (this.missingRanges.length > 0) {
                        ...
                        if (range_data && range_data.start && range_data.end) {
    ...
    async addLifecyclePolicies(indexConfig) {
        if (indexConfig.ILPs) {
    ...
    async appendExtraMappings(indexConfig) {
        // Modify mappings
    ...
    async updateIndexTemplates(indicesList, indexConfig) {
    ...
    async createIndices(indicesList) {
        // Create indices
        const queue_prefix = this.conf.settings.chain;
    ...
    async setupStreaming() {
        ...
        if (_streaming.enable) {
            ...
            if (_streaming.deltas)
                hLog('Delta streaming enabled!');
            if (_streaming.traces)
                hLog('Action trace streaming enabled!');
    ...
    setupDSElogs() {
        ...
        if (existsSync(dsErrorsLog))
            unlinkSync(dsErrorsLog);
        const symbolicLink = logPath + '/deserialization_errors.log';
        if (existsSync(symbolicLink))
            unlinkSync(symbolicLink);
    ...
    reportMissedBlocks(missingProd, lastProducedBlockNum, size) {
    ...
    async checkQueues(autoscaleConsumers, limit) {
    ...
    async runMaster() {
        ...
        this.emitAlert('info', `Range defined from ${this.starting_block} to ${this.head}` +
    ...
    async streamBlockHeaders(start_on, stop_on, func) {
        const responseQueue = [];
        ...
        if (init_response) {
            if (typeof init_response.hits.total !== "number") {
                if (init_response.hits && init_response.hits.total) {
    ...
    addPm2Actions() {
            ...
            try {
                ...
                setInterval(() => {
                    if (this.allowShutdown) {
                        getLastIndexedBlockFromRange(this.client, this.chain, this.starting_block, this.head).then((value) => {
        ...
        pm2io.action('get_heap', (reply) => {
            ...
            }, 'v8_heap_report').then(value => {
                reply(value);
            });
        });
    }
    async requestDataFromWorkers(requestEvent, responseEventType, timeoutSec = 1000) {
        const requests = [];
