...
import got from "got";
function addRedirect(server, url, redirectTo) {
    server.route({
        url,
        method: 'GET',
        schema: {
...
function addRoute(server, handlersPath, prefix) {
...
export function registerRoutes(server) {
    ...
    server.route({
        ...
        handler: async (request, reply) => {
            const data = await got.get(`${server.chain_api}/v1/node/get_supported_apis`).json();
    ...
    if (server.manager.config.features.streaming) {
        // steam client lib
        server.get('/stream-client.js', { schema: { tags: ['internal'] } }, (request, reply) => {
