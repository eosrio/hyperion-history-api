...
class HyperionApiServer {
    ...
    constructor() {
        ...
        const extendedActionsSet = new Set([...extendedActions]);
        ...
        let chainApiUrl = this.conf.api.push_api;
        ...
        if (!this.conf.api.disable_rate_limit) {
            ...
            if (this.conf.api.rate_limit_allow && this.conf.api.rate_limit_allow.length > 0) {
                const tempSet = new Set(['127.0.0.1', ...this.conf.api.rate_limit_allow]);
    ...
    activateStreaming() {
        ...
        import('./socketManager.js').then((mod) => {
            ...
            this.socketManager = new mod.SocketManager(this.fastify, `http://${_host}:${_port}`, this.manager.conn.redis);
            this.socketManager.startRelay();
        });
    }
    addGenericTypeParsing() {
    ...
    async init() {
        ...
        try {
            ...
            hLog(`${this.chain} hyperion api ready and listening on port ${this.fastify.server.address().port}`);
