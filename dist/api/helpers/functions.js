...
import got from "got";
export function extendResponseSchema(responseProps) {
    const props = {
...
export function extendQueryStringSchema(queryParams, required) {
    const params = {
        limit: {
...
export function mergeActionMeta(action) {
    ...
    if (action['@' + name]) {
        action['act']['data'] = Object.assign(action['act']['data'], action['@' + name]);
...
export function mergeDeltaMeta(delta) {
    const name = delta.table;
    if (delta["@" + name]) {
        delta['data'] = Object.assign(delta['data'], delta['@' + name]);
...
export function setCacheByHash(fastify, hash, response, expiration) {
...
export function getRouteName(filename) {
    const arr = filename.split("/");
    return arr[arr.length - 2];
}
export function addApiRoute(fastifyInstance, method, routeName, routeBuilder, schema) {
...
export async function getCachedResponse(server, route, key) {
...
function bigint2Milliseconds(input) {
...
export async function timedQuery(queryFunction, fastify, request, route) {
    // get reference time in nanoseconds
    const t0 = process.hrtime.bigint();
    // check for cached data, return the response hash if caching is enabled
    const [cachedResponse, hash] = await getCachedResponse(fastify, route, request.method === 'POST' ? request.body : request.query);
...
export function chainApiHandler(fastify) {
...
export async function handleChainApiRedirect(request, reply, fastify) {
...
export function addChainApiRoute(fastify, routeName, description, props, required) {
    const baseSchema = {
        description: description,
        summary: description,
        tags: ['chain']
    };
    addApiRoute(fastify, ['GET', 'HEAD'], routeName, chainApiHandler, {
        ...baseSchema,
        querystring: props ? {
            type: 'object',
            properties: props,
    ...
    addApiRoute(fastify, 'POST', routeName, chainApiHandler, {
        ...baseSchema,
        body: props ? {
            type: ['object', 'string'],
            properties: props,
...
export function addSharedSchemas(fastify) {
    fastify.addSchema({
        $id: "WholeNumber",
        title: "Integer",
