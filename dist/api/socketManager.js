...
async function addBlockRangeOpts(data, search_body, fastify) {
    let timeRange;
    let blockRange;
    let head;
...
async function streamPastDeltas(fastify, socket, data) {
    const search_body = {
        query: { bool: { must: [] } },
        sort: {
            block_num: {
...
async function streamPastActions(fastify, socket, data) {
    const search_body = {
        query: { bool: { must: [] } },
        sort: {
            global_sequence: {
...
export class SocketManager {
    ...
    url;
    server;
    constructor(fastify, url, redisOptions) {
        this.server = fastify;
        this.url = url;
