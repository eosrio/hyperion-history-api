...
function buildActionBulk(payloads, messageMap, maxBlockCb, routerFunc, indexName) {
    return flatMap(payloads, (payload, body) => {
...
function buildBlockBulk(payloads, messageMap, maxBlockCb, routerFunc, indexName) {
    return flatMap(payloads, (payload, body) => {
...
function buildDeltaBulk(payloads, messageMap, maxBlockCb, routerFunc, indexName) {
    let maxBlock = 0;
...
export class ElasticRoutes {
    ...
    ingestNodeCounters = {};
    distributionMap;
    constructor(connectionManager, distributionMap) {
    ...
    async handleGenericRoute(payload, ch, cb) {
        const coll = {};
    ...
    onResponse(resp, messageMap, callback, payloads, channel, index_name, maxBlockNum) {
        if (resp.errors) {
            this.ackOrNack(resp, messageMap, channel);
            if (maxBlockNum > 0) {
                ElasticRoutes.reportMaxBlock(maxBlockNum, index_name);
    ...
    routeFactory(indexName, bulkGenerator, routerFunction) {
    ...
    addRoute(indexType, bulkGenerator, routerFunction) {
    ...
    registerDynamicTableRoute() {
        this.routes['dynamic-table'] = async (payloads, channel, cb) => {
            ...
            for (const entry of contractMap.entries()) {
                processingQueue.push(this.createGenericBuilder(entry[1], channel, counter, `${this.chain}-dt-${entry[0]}`, buildDynamicTableBulk));
            }
    ...
    static reportMaxBlock(maxBlockNum, index_name) {
        process.send?.({
