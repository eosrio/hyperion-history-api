...
export async function getCacheByHash(redis: Redis, key: string, chain: string): Promise<[string | null, string]> {
    const hash = createHash('sha256');
    const query_hash = hash.update(chain + "-" + key).digest('hex');
    return [await redis.get(query_hash), query_hash];
}

export function mergeActionMeta(action: Record<string, any>): Record<string, any> {
...
export function mergeDeltaMeta(delta: Record<string, any>): Record<string, any> {
