...
class HyperionApiServer {
  ...
  constructor() {
    ...
    hLog(
      `Chain API URL: "${this.fastify.chain_api}" | Push API URL: "${this.fastify.push_api}"`,
    );
    ...
    if (!this.conf.api.disable_rate_limit) {
      ...
      if (
        this.conf.api.rate_limit_allow &&
        this.conf.api.rate_limit_allow.length > 0
      ) {
    ...
    const headerLogo = readFileSync(
      join(staticAssetsDir, 'logo-hyperion-white-horz.svg'),
    );

    const customCss = readFileSync(
      join(staticAssetsDir, 'theme', 'custom-swagger-theme.css'),
    ).toString();
  ...
  activateStreaming() {
    ...
    if (_host === '0.0.0.0') {
      hLog(
        `[ERROR] WS Router Host is set to 0.0.0.0, please use a fixed IP address instead. Can't start streaming.`,
      );
      return;
    }
  ...
  async init() {
    ...
    await waitUntilReady(
      async () => {
        try {
          ...
          hLog(
            `Elasticsearch: ${esInfo.version.number} | Lucene: ${esInfo.version.lucene_version}`,
          );
    ...
    try {
      ...
      if (listeningAddress.address === '0.0.0.0') {
        hLog(
          `Hyperion API for "${this.chain}" ready and listening on port ${listeningAddress.port} (all interfaces)`,
        );
      } else {
        hLog(
          `Hyperion API for "${this.chain}" ready and listening on port http://${listeningAddress.address}:${listeningAddress.port}`,
        );
      }
  ...
  registerQryHubRoutes() {
    ...
    this.fastify.get('/.qry/first', {schema: {hide: true}}, async () => {
      ...
      const firstBlock = await getFirstIndexedBlock(
        ...
        this.chain,
        this.conf.settings.index_partition_size,
      );
    ...
    this.fastify.get(
      ...
      async (request, reply) => {
        ...
        const blockChunkSize =
          this.conf.settings.index_partition_size || 10000000;
        ...
        const results = await elastic.search<any>({
          ...
          query: {
            range: {
              block_num: {
                gte: firstBucket.key,
                lt: firstBucket.key + histogramInterval,
              },
            },
          },
      ...
      if (recentBucket) {
        if (this.lastSentTimestamp !== recentBucket.timestamp) {
          ...
          if (this.qryPublisher) {
            this.qryPublisher.publishApiUsage(
              totalValidHits,
              recentBucket.timestamp,
            );
          }
  ...
  setupIndexerController() {
    ...
    let controlPort =
      this.manager.conn.chains[this.conf.settings.chain].control_port;
    if (!controlPort) {
      controlPort = 7002;
    }
  ...
  private initAlerts() {
    ...
    if (alertManagerConf) {
      import('../indexer/modules/alertsManager.js')
        .then((module) => {
        ...
        if (this.manager.config.api.explorer.theme) {
          ...
          if (existsSync(themesPath)) {
            const themeFile = join(
              themesPath,
              this.manager.config.api.explorer.theme + '.theme.mjs',
            );
