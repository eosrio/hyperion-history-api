...
async function getDeltas(fastify: FastifyInstance, request: FastifyRequest) {
    ...
    // Cold-tier archive hydration for deltas: re-fetch any dropped data/value from the configured
    // delta archive (api.archives.deltas) before the response is shaped. Best-effort — failures leave
    // the delta without data/value and never fail the request. Skipped via ?hydrate=false, and a clean
    // no-op when no delta archive is configured (behaves exactly as before).
