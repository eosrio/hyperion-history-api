...
interface GetBlockResultsWithTransactions extends RpcInterfaces.GetBlockResult {
    transactions: any[];
}
...
async function getCreator(fastify: FastifyInstance, request: FastifyRequest) {
    ...
    const results = await fastify.elastic.search<any>({
        ...
        size: 1,
        query: {bool: {must: [{term: {"@newaccount.newact": query.account}}]}}
    });
        ...
        if (accountInfo) {
            try {
                ...
                const blockHeader = await fastify.elastic.search<any>({
                    ...
                    size: 1,
                    query: {
                        bool: {must: [{term: {"@timestamp": response.timestamp}}]}
                    }
                });
                if (blockHeader.hits.hits && blockHeader.hits.hits.length > 0 && blockHeader.hits.hits[0]._source) {
                    const blockId = blockHeader.hits.hits[0]._source.block_id;
                    const blockData = await fastify.eosjs.rpc.get_block(blockId) as GetBlockResultsWithTransactions;
