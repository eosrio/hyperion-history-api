jq --arg elastic_user $elastic_user --arg  elastic_pass $elastic_pass --arg elastic_host $elastic_host
    '.elasticsearch.user = $elastic_user | .elasticsearch.pass =  $elastic_pass | .elasticsearch.host = [$elastic_host] |
    .chains.voice.http = $chain_http ' /tmp/connections.json > /hyperion-history-api/connections.json
