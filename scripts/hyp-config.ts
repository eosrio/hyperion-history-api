...
const exampleConnectionsPath = path.join(
  path.resolve(),
...
async function getExampleConfig(): Promise<HyperionConfig> {
  const exampleChainData = await readFile(
    path.join(chainsDir, "example.config.json")
  );
          ...
          for (const subKey in exampleChain[key]) {
            if (!jsonData[key].hasOwnProperty(subKey)) {
              if (flags.fixMissingFields) {
                ...
                console.log(
                  `⚠ Field ${subKey} missing from ${key} on ${file} (default = ${exampleChain[key][subKey]})`
                );
                missingFields++;
              }
            }
...
async function newChain(shortName, options) {
  ...
  if (!connections) {
    console.log(
      `The connections.json file is not present, please run "./hyp-config connections init" to configure it.`
    );
    process.exit(1);
  }
  ...
  if (options.http) {
    if (
      options.http.startsWith("http://") ||
      options.http.startsWith("https://")
    ) {
      console.log(`Testing connection on ${options.http}`);
    } else {
      console.error(
        `Invalid HTTP Endpoint [${options.http}] - Url must start with either http:// or https://`
      );
      process.exit(1);
    }
  ...
  if (options.ship) {
    if (options.ship.startsWith("ws://") || options.ship.startsWith("wss://")) {
      ...
      console.error(
        `Invalid WS Endpoint [${options.ship}] - Url must start with either ws:// or wss://`
      );
      process.exit(1);
    }
...
async function rmChain(shortName) {
  ...
  try {
    ...
    await cp(
      targetPath,
      path.join(backupDir, `${shortName}_${now}_config.json`)
    );
  } catch (e) {
    console.log(e.message);
...
async function checkAMQP(conn: HyperionConnections): Promise<boolean> {
  ...
  const amqp_url = `amqp${conn.amqp.secure ? "s" : ""}://${u}:${p}@${
    conn.amqp.host
  }/${v}?frameMax=${frameMaxValue}`;
  try {
    ...
    if (connection) {
      ...
      headers.set(
        "Authorization",
        "Basic " + btoa(conn.amqp.user + ":" + conn.amqp.pass)
      );
      ...
      if (resp.length > 0) {
        for (const node of resp) {
          console.log(
            `[info] [RABBIT:HTTP] - Node name: ${node.name} (running: ${node.running})`
          );
          if (!node.running) {
            return false;
          }
...
async function initConfig() {
  ...
  while (!amqp_state) {
    ...
    if (!amqp_state) {
      ...
      const amqp_pass = await prompt(
        ' > Enter the RabbitMQ password (or press ENTER to use "hyperion_password"): '
      );
      if (amqp_pass) {
      ...
      const amqp_vhost = await prompt(
        ' > Enter the RabbitMQ vhost (or press ENTER to use "hyperion"): '
      );
      if (amqp_vhost) {
  ...
  while (!redis_state) {
    ...
    if (!redis_state) {
      ...
      const redis_port = await prompt(
        ' > Enter the Redis Server port (or press ENTER to use "6379"): '
      );
      if (redis_port) {
  ...
  try {
    ...
    if (existsSync(connectionsPath)) {
      ...
      const confirmation = await prompt(
        'Are you sure you want to reset the connection configuration? Type "YES" to confirm.\n'
      );
      if (confirmation === "YES") {
        copyFileSync(
          connectionsPath,
          path.join(backupDir, "connections.json.bak")
        );
        rmSync(connectionsPath);
        console.log(
          'connections.json removed, please use "./hyp-config connections init" to reconfigure'
        );
      } else {
