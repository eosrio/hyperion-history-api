...
### Path forward
1. **`trace_history` → actions** (`<chain>-action-v1`) — **PROTOTYPED** (`action-proto`, branch `proto/delta-indexer`, 2026-05-31). The biggest, most-queried index. `trace_history.{log,index}` shares `chain_state_history`'s `state_history_log` framing verbatim (confirmed on the WAX node: identical magic, same `first_block=2`, byte-identical index size), so the engine is reused as-is. **Two-level decode:** L1 decodes the whole block's `transaction_trace[]` in one `AbiHandle::bin_to_json_into` against an **embedded SHiP protocol ABI** (`abis/ship.abi.json`, a WAX `eosio::abi/1.1` capture — nodeos emits it only over the websocket, so offline decode must embed it; the rust-backend handles the top-level `[]`, the `action_trace` v0/v1 variant, optionals, and the recursive `failed_dtrx_trace`); L2 decodes each `act.data` against the contract ABI active at the block, reusing delta-proto's per-worker `AbiHandle` Registry. Faithful ports of Hyperion `groupActionTraces` (notification receipts merged into `receipts[]`, `act_digest`/`code_sequence`/`abi_sequence` hoisted) and `cleanActionTrace`. **Validated on WAX 190,373,745–190,376,745** (3,001 blocks; 364,063 txs; 2,027,964 action_traces → 1,220,043 grouped docs): **99.99% of `act.data` decoded** version-correctly — the 0.01% are `eosio.null::nonce` (no ABI, exactly as Hyperion). Warm 8-thread ~836 blk/s. **Phase B + full parity DONE (2026-05-31):** a minimal nodeos block-log reader (`src/blocks.rs` — `blocks.index` offset/block; `signed_block`'s `block_header` prefix = `block_timestamp_type` slot + `producer` name) supplies `@timestamp`+producer; all `@`-field handlers (transfer + eosio system actions) ported. **Validated against live WAX Hyperion (api.waxsweden.org) on block 437,420,000: 91/91 docs, ZERO field diffs** — block fields, producer, `@timestamp`, receipt grouping, ordinals, authorization, `act.data` byte-exact on the 89 non-transformed actions and `@`-field values exact on the rest. (Decode parity is structural: rs_abieos's rust-backend is differentially tested against the same C++ abieos `@eosrio/node-abieos` wraps.) **Throughput optimization — hand-walk DONE (~2.6×).** Replaced the SHiP-ABI L1 decode (full-trace→JSON→serde-reparse, act.data rendered as hex then re-decoded) with a zero-copy hand-walk of the `transaction_trace[]` binary (`src/trace.rs`) yielding only the needed fields + `act.data` as a raw byte range (no hex round-trip; the embedded SHiP ABI is no longer needed and was removed). Scalar renderings match rs_abieos exactly, so output is **byte-identical** — verified by A/B `cmp` against the rs_abieos path (IDENTICAL on 205,734 docs / 500 blocks and the 91-doc validated block). Warm 8-thread **804 → ~2,115 blk/s**; the binary walk fell from 46% of the time to ~1.4%. A follow-up **string-builder** doc emit (static `&str` keys written straight to a `String`, raw decoded `act.data` spliced verbatim for non-`@`-handled actions; `@`-handled ones still parse → existing handler → reserialize) then took it to **~3,503 blk/s (~4.4× total over the session)** — and is *more* faithful: a parsed-compare over 205,670 docs found only 23 diffs, all float precision where the Map path was silently round-tripping floats through serde's ryu while the raw splice preserves rs_abieos's `format_finite_float_json` (matched to C++ abieos = node-abieos = Hyperion). Re-validated 91/91, 0-diff vs live Hyperion. Remaining decode runway is small and diminishing (per-name buffer reuse, ~1.3×); the real backfill ceiling is now **ES `_bulk`**, so the next high-value step is wiring the reader into the ingestor/ES path (it emits the same `<chain>:index_actions:N` AMQP the ingestors consume — replacing reader+deserializer+ds_pool, no extra deserializer fan-out) to measure the true end-to-end ceiling.
